...
const string strMessageMagic = "Ultracoin Signed Message:\n";
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
    ...
    {
        // Maximum 200% adjustment per day... (30mins now)
        bnResult *= 2;
        //nTime -= 24 * 60 * 60;
		nTime -= 30 * 60;
    }
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	//minimum diff 1 after block 2
	//if (bnNew > bnInitialHashTarget)
    //    bnNew = bnInitialHashTarget;
...
// fix PoS issues
{
    ...
    if (nHeight >= nConsecutiveStakeSwitchHeight) {
        ...
        if (IsProofOfWork()) {
            ...
            CBigNum bnTrust = bnInitialHashTarget / bnTarget;
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Ultracoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
  ...
  //vMerkleTree: 05b4eb8ba8 
        ...
        // Genesis block
        const char* pszTimestamp = "Ultracoin to the Moon";
        CTransaction txNew;
